        ...
        public static GraphUserAccount ToGraphUserAccount(this ClaimsPrincipal claimsPrincipal)
        {
            if (claimsPrincipal == null)
            {
                throw new AuthenticationException(
                    new Error
                    {
                        ...
                        Message = string.Format(ErrorConstants.Messages.NullValue, "claimsPrincipal")
                    });
            }
            ...
            if (string.IsNullOrEmpty(objectId))
            {
                throw new AuthenticationException(
                    new Error
                    {
                        ...
                        Message = string.Format(ErrorConstants.Messages.MissingClaim, AuthConstants.ClaimTypes.ObjectIdUriSchema)
                    });
            }
                

            string tenantId = claimsPrincipal.FindFirst(AuthConstants.ClaimTypes.TenantIdUriSchema)?.Value;
            if (string.IsNullOrEmpty(tenantId))
            {
                tenantId = claimsPrincipal.FindFirst(AuthConstants.ClaimTypes.TenantIdJwt)?.Value;
            }
            ...
            if (string.IsNullOrEmpty(tenantId))
            {
                throw new AuthenticationException(
                    ...
                    {
                        Code = ErrorConstants.Codes.InvalidClaim,
                        Message = string.Format(ErrorConstants.Messages.MissingClaim, AuthConstants.ClaimTypes.TenantIdUriSchema)
                    });
            }
            ...
            return new GraphUserAccount() { Email = email, ObjectId = objectId, TenantId = tenantId };
        }
    }
}